...
bool WindowStack::pointInWindow(const MPoint<int>& point) const
{
	for (auto* window : mWindowList)
	{
...
void WindowStack::updateStack(const MPoint<int>& point)
{
	for (auto* window : mWindowList)
	{
		if (window->visible() && window->area().contains(point))
...
void WindowStack::injectMouseMotion(const MPoint<int>& delta)
{
	for (auto* window : mWindowList)
	{
		if(window->visible())
...
void WindowStack::reposition(float prevScale, float scale)
{
	for (auto* window : mWindowList)
	{
		window->position(MPoint<int> {
			(int)roundf(window->position().x * scale / prevScale),
			(int)roundf(window->position().y * scale / prevScale)
		});
	}
}