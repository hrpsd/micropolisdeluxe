...
namespace
{
    ...
    std::map<Tool, ToolProperties> Tools =
    {
        ...
        { Tool::Bulldoze, { 1, 1, 0, true, "Bulldoze" }},
...
void toolStart(const MPoint<int>& start)
{
    ToolStart = start;
}
...
void toolEnd(const MPoint<int>& end)
{
    ToolEnd = end;
}
...
ToolResult checkArea(const int mapH, const int mapV, const int base, const int size, const bool animate, const Tool tool, Budget& budget)
{
    ...
    for (int row = 0; row < size; ++row)
    {
        ...
        for (int col = 0; col < size; ++col)
        {
            ...
            else if (gameOptions.mAnimation && col == 1 && row == 2)
            {
    ...
    for (int x = mapX - 1; x < mapX + size - 1; x++)
    {
        for (int y = mapY - 1; y < mapY + size - 1; y++)
        {
            if (CoordinatesValid({ x, y }))
            {
                ...
                {
                    map[x][y] = (gameOptions.mAnimation ? (TINYEXP + RandomRange(0, 2)) : SOMETINYEXP) | ANIMBIT | BULLBIT;
                }
            }
        }
    }
...
void ToolDown(const MPoint<int> location, Budget& budget)
{
    ...
    else if (result == ToolResult::Ignore)
    {
    }
    else
    {
...
void validateDraggableToolVector(Vector<int>& toolVector, Budget& budget)
{
    ...
    const MPoint<int>& origin = toolStart();
...
void executeDraggableTool(const Vector<int>& toolVector, const MPoint<int>& tilePointedAt, Budget& budget)
{
